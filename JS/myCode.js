...
class SpriteAnimator {
  ...
  draw(ctx, cx, cy, scale = 1, isMirrored = false) {
    ...
    ctx.drawImage(
      ...
      0,
      anim.frameW,
      anim.frameH, // Source rect (one frame)
      -dw / 2,
      -dh,
      dw,
      dh, // Destination: centre-x=0, bottom=0
    );
    ctx.restore();
  }
...
const MAPS = [
  {
    ...
    ambientConfig: {
      ...
      color: [
        ...
        "rgba(143,193,227,0.65)",
        "rgba(224,247,250,0.55)",
      ],
      fallSpeed: 0.35,
      driftSpeed: 0.45,
      minSize: 0.8,
      ...
      },
      {
        ...
        draw(ctx, ox, W, H) {
          ...
          [80, 300, 550, 820, 1100].forEach((cp, i) => {
            ...
            ctx.fill();
            ctx.beginPath();
            ctx.arc(cx + r * 0.6, cy - 20, r * 0.7, 0, Math.PI * 2);
            ctx.fill();
          });
        },
      },
  ...
  },
  {
    ...
    drawDecorations(ctx) {
      ...
      ctx.beginPath();
      ctx.arc(640, 550, 30, 0, Math.PI * 2);
      ctx.fill();
    },
  },
];
...
class ParticleSystem {
  ...
  spawn(x, y, count, config) {
    for (let i = 0; i < count; i++) {
      const angle =
        config.angle !== undefined ? config.angle : Math.random() * Math.PI * 2;
...
class Fighter {
  ...
  constructor(game, id, charData, x, y, isPlayer, controls = {}) {
    ...
    this.y = y;
    this.width = 40;
    this.height = 72; // Hurtbox dimensions (px)
    this.vx = 0;
    this.vy = 0; // Velocity (px/frame)
  ...
  getHitbox() {
    ...
    return {
      ...
      baseKnockback: isHeavy
        ? this.charData.heavyKnockback
        : this.charData.lightKnockback,
  ...
  _resolveAnimation() {
    ...
    if (this.hitstunTime > 0) {
      const key =
        id === "WIZARD" ? "Hit" : id === "MARTAIL" ? "Take Hit" : "Take hit";
  ...
  handleInput(keys) {
    ...
    if (dashKey && !this.prevDashKey && this.dashCooldown <= 0) {
      ...
      this.vy = 0;
      audio.playDash();
      this.game.particles.spawnGlow(this.x, this.y - this.height * 0.45, 14, {
        minSpeed: 2,
        maxSpeed: 8,
        minSize: 3,
        maxSize: 9,
        color: [this.charData.accent, "#ffffff"],
        decay: 0.055,
      });
    }
    ...
    if (this.jumpBufferFrames > 0) {
      const coyoteOk =
        this.coyoteFrames > 0 && this.jumpsLeft === this.charData.maxJumps;
  ...
  triggerAttack(type) {
    ...
    if (type === "HEAVY") {
      ...
      this.game.particles.spawnSwipe(
        ...
        this.charData.attackRange * 0.72,
        sA,
        eA,
        this.charData.accent,
      );
    } else {
      ...
      this.game.particles.spawnSwipe(
        ...
        this.charData.attackRange * 0.54,
        sA,
        eA,
        "#ffffff",
      );
  ...
  _isOffStage() {
    return this.x < 60 || this.x > 1220 || this.y > 640;
  }

  /** @private True when close enough to an edge that recovery is wise. */
  _isDangerZone() {
    return this.x < 210 || this.x > 1070;
  }
  ...
  update(platforms) {
    ...
    if (!this.isGrounded) {
      ...
      const maxFall = this.fastFalling
        ? PHYS.MAX_FAST_FALL
        : PHYS.MAX_FALL_SPEED;
  ...
  draw(ctx) {
    ...
    if (this.emoteTimer > 0) {
      ...
      const fadeIn = Math.min(
        1,
        this.emoteTimer > totalFrames - 10
...
class PostProcessor {
  ...
  apply(ctx, canvas) {
    ...
    try {
      ...
      /* Cross-origin canvas guard — silently skip */
    }
...
class Engine {
  ...
  updateHtmlHud(show) {
    ...
    const c1 = CHARACTERS[this.p1CharIndex],
      c2 = CHARACTERS[this.p2CharIndex];
    ...
    if (n2)
      n2.innerText = `${this.playersCount === 2 ? "P2" : "CPU"} (${c2.name.toUpperCase()})`;
  }
  ...
  drawParallaxBackground() {
    const W = this.canvas.width,
      H = this.canvas.height;
    const gc =
      (this.state === "PLAYING" || this.state === "PAUSED") && this.map
        ? this.map.bgGradient
        : ["#0f172a", "#1e293b", "#334155"];

    const bg = this.ctx.createLinearGradient(0, 0, 0, H);
    bg.addColorStop(0, gc[0]);
    bg.addColorStop(0.5, gc[1]);
    bg.addColorStop(1, gc[2]);
    this.ctx.fillStyle = bg;
    this.ctx.fillRect(0, 0, W, H);

    if (
      (this.state === "PLAYING" || this.state === "PAUSED") &&
      this.map?.parallaxLayers
    ) {
      const offX = this.camera.worldOffsetX;
      this.map.parallaxLayers.forEach((l) =>
        l.draw(this.ctx, offX * l.depth, W, H),
      );
    } else {
  ...
  drawButton(x, y, w, h, text, hov, col = "#10b981") {
    ...
    hov
      ? (g.addColorStop(0, col), g.addColorStop(1, "#059669"))
  ...
  drawHomeScreen() {
    ...
    this.ctx.fillText(
      "SUPER PREMIUM CANVAS PLATFORM FIGHTER",
      this.canvas.width / 2,
      318,
    );
    ...
    const s = this.registerZone(500, 410, 280, 65, () => {
      this.state = "MODE_SELECT";
    });
    this.drawButton(500, 410, 280, 65, "🚀 START GAME", s, "#e94560");
    const st = this.registerZone(500, 500, 280, 65, () => {
      this.state = "SETTINGS";
    });
  ...
  drawSettingsScreen() {
    ...
    this.ctx.fillText(
      `Match Duration: ${this.matchDuration / 60} Minutes`,
      400,
      280,
    );
    ...
    const bh = this.registerZone(500, 550, 280, 60, () => {
      this.state = "HOME";
    });
  ...
  drawModeSelectScreen() {
    ...
    this._modeCard(
      ...
      250,
      "🤖 1 PLAYER",
      "Battle a reactive CPU opponent.",
      p1h,
      "#3a86ff",
    );
    ...
    this._modeCard(
      ...
      250,
      "👥 2 PLAYER",
      "Local dual-keyboard duel.",
      p2h,
      "#e94560",
    );

    const bh = this.registerZone(500, 580, 280, 50, () => {
      this.state = "HOME";
    });
  ...
  drawHeroSelectScreen() {
    ...
    this.ctx.fillText(
      this.activeSelectingPlayer === 1
        ? "PLAYER 1 SELECT HERO"
        : "PLAYER 2 SELECT HERO",
      this.canvas.width / 2,
      80,
    );
    ...
    for (let i = 0; i < total; i++) {
      ...
      this.ctx.beginPath();
      this.ctx.roundRect(cx, cy, cw, ch, 16);
      this.ctx.fill();
      this.ctx.stroke();
    ...
    this.ctx.fillText(
      `✦ P1: ${CHARACTERS[this.p1CharIndex].name}`,
    ...
    const p2Img = CHARACTERS[this.p2CharIndex]
      ? this.menuImages[this.p2CharIndex]
      : null;
  ...
  drawMapSelectScreen() {
    ...
    for (let i = 0; i < MAPS.length; i++) {
      ...
      this.ctx.beginPath();
      this.ctx.roundRect(ax, ay, tw, th, 16);
      this.ctx.fill();
      this.ctx.stroke();
      ...
      this.ctx.save();
      this.ctx.beginPath();
      this.ctx.roundRect(ax + 15, ay + 15, tw - 30, 185, 10);
      this.ctx.clip();
      const tg = this.ctx.createLinearGradient(ax, ay, ax, ay + 185);
      tg.addColorStop(0, m.bgGradient[0]);
      tg.addColorStop(1, m.bgGradient[1]);
      this.ctx.fillStyle = tg;
      this.ctx.fillRect(ax + 15, ay + 15, tw - 30, 185);
      const ms = (tw - 60) / 1000,
        tc = ax + tw / 2;
      m.platforms.forEach((p) => {
        ...
        const mx = tc + rx * ms,
          my = ay + 100 + ry * ms * 0.8;
        const mw = p.width * ms,
          mh = Math.max(6, p.height * ms);
        this.ctx.fillStyle = p.color;
        this.ctx.beginPath();
        this.ctx.roundRect(mx, my, mw, mh, 3);
        this.ctx.fill();
    ...
    const bh = this.registerZone(500, 575, 280, 50, () => {
      this.state = "HERO_SELECT";
    });
  ...
  drawPlayingScreen() {
    ...
    if (
      h1 &&
      !this.p1.hasDealtDamageThisAttack &&
      this._aabb(h1, this.p2.getHurtbox())
    ) {
      if (this.p2.applyHit(h1, this.p1))
        this.p1.hasDealtDamageThisAttack = true;
    }
    ...
    if (
      h2 &&
      !this.p2.hasDealtDamageThisAttack &&
      this._aabb(h2, this.p1.getHurtbox())
    ) {
      if (this.p1.applyHit(h2, this.p2))
        this.p2.hasDealtDamageThisAttack = true;
    }
    ...
    for (const p of this.map.platforms) {
      ...
      this.ctx.beginPath();
      this.ctx.roundRect(p.x, p.y, p.width, p.height, p.soft ? 4 : 8);
      this.ctx.fill();
      this.ctx.shadowBlur = 0;
      this.ctx.fillStyle = p.topColor;
      this.ctx.beginPath();
    ...
    this.ctx.textAlign = "right";
    this.ctx.fillText(
      `📷 Zoom: ${this.camera.scale.toFixed(2)}x`,
      this.canvas.width - 8,
      18,
    );
    this.ctx.restore();
    ...
    if (this.p1.stocks <= 0 || this.p2.stocks <= 0 || timeUp) {
      this.winner =
        this.p1.stocks > this.p2.stocks
          ? 1
          : this.p2.stocks > this.p1.stocks
            ? 2
            : this.p1.damagePercent < this.p2.damagePercent
              ? 1
              : 2;
      audio.playDeath();
  ...
  drawPausedScreen() {
    ...
    const rh = this.registerZone(500, 380, 280, 55, () => {
      this.state = "PLAYING";
      audio.playBlip();
    });
    this.drawButton(500, 380, 280, 55, "▶ RESUME", rh, "#06d6a0");
    const qh = this.registerZone(500, 460, 280, 55, () => {
      this.state = "HOME";
      audio.playSelect();
    });
  ...
  drawGameOverScreen() {
    ...
    this.ctx.beginPath();
    this.ctx.roundRect(400, 300, 480, 175, 16);
    this.ctx.fill();
    ...
    const rh = this.registerZone(480, 508, 320, 58, () => {
      this.startNewMatch();
    });
    this.drawButton(480, 508, 320, 58, "🔄 REMATCH", rh, "#06d6a0");
    const hh = this.registerZone(480, 586, 320, 48, () => {
      this.state = "HOME";
    });
  ...
  animate() {
    ...
    switch (this.state) {
      ...
      case "SETTINGS":
        ...
        break;
      case "MODE_SELECT":
        this.drawModeSelectScreen();
        break;
      case "HERO_SELECT":
        this.drawHeroSelectScreen();
        break;
      case "MAP_SELECT":
        this.drawMapSelectScreen();
        break;
      case "PLAYING":
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "  New in v3.0: bloom particles, flash telegraph, damped-sine shake,",
  );
  console.log(
    "  character emotes (E / /), ambient debris, center-crop portraits.",
  );
});
